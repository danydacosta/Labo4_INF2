...
Fraction<T> Fraction<T>::simplifier() const {
   T pgdc = plusGrandDiviseurCommun(T(std::abs(numerateur)), denominateur);
   return Fraction<T>(T(numerateur / pgdc), T(denominateur / pgdc));
}
 ...
 * si c'est le cas
 * @param MSG_ERREUR message d'erreur à afficher
...
Fraction<T>& Fraction<T>::operator+=(const Fraction<T> &rhs) {
   ...
   T facteurGauche = T(tmpMembreDroite.denominateur / denomPGDC);
   T facteurDroite = T(tmpMembreGauche.denominateur / denomPGDC);
